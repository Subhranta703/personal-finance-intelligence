...
export async function createBudget(req:Request,res:Response){const {category,month,amount,threshold=80}=req.body;const b=await Budget.findOneAndUpdate({userId:req.userId,category,month},{userId:req.userId,category,month,amount,threshold},{upsert:true,new:true,setDefaultsOnInsert:true});res.status(201).json(b)}

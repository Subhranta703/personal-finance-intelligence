...
setError(false);
  ...
  const [overview, anomalyData, userData, transactionData] =
    await Promise.all([
...
if (loading) {
return ( <div className="space-y-5"> <div className="h-10 w-64 rounded-xl bg-[var(--surface2)] animate-pulse" />
...
if (error || !data) {
return ( <div className="min-h-[60vh] grid place-items-center"> <div className="card max-w-md p-8 text-center"> <div className="mx-auto h-12 w-12 rounded-2xl bg-orange-500/10 grid place-items-center"> <AlertTriangle
           size={24}
...
const maxCategoryValue = Number(cats[0]?.[1] || 1);
...
========================== */}
    ...
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--muted)]">
          Here’s the financial picture from your latest data.
        </p>
      </div>
    </div>
  </FadeIn>
  ...
  <div className="mt-7 grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
    {[
      ["Net savings", data.savings, "teal", "Current"],
      ["Income", data.income, "emerald", "This month"],
      ["Expenses", data.expenses, "coral", "This month"],
      ["Savings rate", data.savingsRate, "indigo", "Current"],
    ].map(([label, val, tone, meta], i) => (
      <FadeIn
        key={String(label)}
        delay={i * 0.05}
      >
        <HoverCard className="card p-5">
          ...
          <div className="mt-4 text-2xl font-black">
            {i === 3 ? (
              ...
              <AnimatedNumber value={Number(val)} />
            )}
          </div>
          ...
          <div className="mt-7">
            <div className="flex items-end gap-4 h-56">
              {cats.map(([name, value]) => {
                ...
                const percentage =
                  (numericValue / maxCategoryValue) * 100;
    ...
    <FadeIn delay={0.25}>
      <div className="card p-5">
        ...
        <div className="mt-6 space-y-5">
          {[
            ["Food", 6200, 8000, "amber"],
            ["Shopping", 4800, 7000, "cyan"],
            ["Transport", 2600, 5000, "emerald"],
          ].map(([n, u, t, c]) => (
            <div key={n as string}>
        ...
        <Link
          href="/budgets"
          className="mt-6 flex items-center justify-between rounded-xl bg-[var(--surface2)] p-3 text-xs font-semibold"
        >
          Manage budgets
            ...
            anomalies.slice(0, 3).map((a) => (
              ...
              >
                ...
                <div className="mt-1 text-sm">
                  ₹
                  {a.amount?.toLocaleString("en-IN")} at{" "}
                  {a.merchant}
                </div>

                <div className="mt-1 text-[11px] text-[var(--muted)]">
                  Score {a.anomalyScore} · indicator only
                </div>
              </div>
            ))
          ) : (
